...
@section('title', 'Modifier l\'article d\'inventaire')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Modifier l'article d'inventaire</h1>
        <p class="text-gray-600 mt-1">Mettez à jour les informations de l'article</p>
    </div>

    <form method="POST" action="{{ route('inventory.update', $inventory->id) }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 border-b border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category" name="category"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->value }}" {{ old('category', $inventory->category) == $category->value ? 'selected' : '' }}>
                                {{ $category->label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="current_stock" name="current_stock" value="{{ old('current_stock', $inventory->current_stock) }}" required step="0.01" min="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="min_stock" name="min_stock" value="{{ old('min_stock', $inventory->min_stock) }}" required step="0.01" min="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="max_stock" name="max_stock" value="{{ old('max_stock', $inventory->max_stock) }}" step="0.01" min="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="unit_price" name="unit_price" value="{{ old('unit_price', $inventory->unit_price) }}" step="0.01" min="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="supplier_name" name="supplier_name" value="{{ old('supplier_name', $inventory->supplier_name) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('notes', $inventory->notes) }}</textarea>
                </div>
            </div>
        </div>
