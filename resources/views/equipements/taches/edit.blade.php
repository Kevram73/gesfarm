...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-1">Mettez à jour les informations de la tâche</p>
    </div>

    <form method="POST" action="{{ route('farm-tasks.update', $task->id) }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('description', $task->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        ...
                        <option value="PLANTING" {{ old('type', $task->type) == 'PLANTING' ? 'selected' : '' }}>Plantation</option>
                        <option value="HARVESTING" {{ old('type', $task->type) == 'HARVESTING' ? 'selected' : '' }}>Récolte</option>
                        <option value="IRRIGATION" {{ old('type', $task->type) == 'IRRIGATION' ? 'selected' : '' }}>Irrigation</option>
                        <option value="FERTILIZATION" {{ old('type', $task->type) == 'FERTILIZATION' ? 'selected' : '' }}>Fertilisation</option>
                        <option value="PEST_CONTROL" {{ old('type', $task->type) == 'PEST_CONTROL' ? 'selected' : '' }}>Lutte contre les ravageurs</option>
                        <option value="MAINTENANCE" {{ old('type', $task->type) == 'MAINTENANCE' ? 'selected' : '' }}>Maintenance</option>
                        <option value="OTHER" {{ old('type', $task->type) == 'OTHER' ? 'selected' : '' }}>Autre</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="CANCELLED" {{ old('status', $task->status) == 'CANCELLED' ? 'selected' : '' }}>Annulée</option>
                    </select>
                </div>
