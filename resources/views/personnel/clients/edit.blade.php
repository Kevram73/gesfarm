...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-1">Mettez à jour les informations du client</p>
    </div>

    <form method="POST" action="{{ route('customers.update', $customer->id) }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        ...
                        <option value="INDIVIDUAL" {{ old('type', $customer->type) == 'INDIVIDUAL' ? 'selected' : '' }}>Individuel</option>
                        <option value="BUSINESS" {{ old('type', $customer->type) == 'BUSINESS' ? 'selected' : '' }}>Entreprise</option>
                        <option value="COOPERATIVE" {{ old('type', $customer->type) == 'COOPERATIVE' ? 'selected' : '' }}>Coopérative</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="is_active" name="is_active"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        <option value="1" {{ old('is_active', $customer->is_active) ? 'selected' : '' }}>Actif</option>
                        <option value="0" {{ !old('is_active', $customer->is_active) ? 'selected' : '' }}>Inactif</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('notes', $customer->notes) }}</textarea>
                </div>
            </div>
        </div>
