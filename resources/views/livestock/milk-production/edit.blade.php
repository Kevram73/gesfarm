...
@section('title', 'Modifier Production Laitière')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('livestock.milk-production.update', $production->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $production->quantity) }}" step="0.01" min="0" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                </div>

                <div>
                    <label for="quality" class="block text-sm font-medium text-gray-700 mb-2">Qualité</label>
                    <input type="text" name="quality" id="quality" value="{{ old('quality', $production->quality) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                </div>
