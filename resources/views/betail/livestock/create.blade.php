...
@section('title', 'Nouveau bétail')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Nouveau bétail</h1>
        <p class="text-gray-600 mt-1">Ajoutez un nouvel animal à votre ferme</p>
    </div>

    <form method="POST" action="{{ route('livestock.store') }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
                ...
                <div>
                    ...
                    <select id="breed" name="breed"
                        ...
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->value }}" {{ old('breed') == $breed->value ? 'selected' : '' }}>
                                {{ $breed->label }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end space-x-4">
            ...
            <button type="submit" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-semibold">
                <i class="fas fa-save mr-2"></i> Créer le bétail
            </button>
        </div>
    </form>
</div>
