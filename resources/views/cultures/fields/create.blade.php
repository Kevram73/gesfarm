...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Nouveau champ</h1>
        <p class="text-gray-600 mt-1">Ajoutez un nouveau champ à votre ferme</p>
    </div>

    <form method="POST" action="{{ route('fields.store') }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($soilTypes as $soilType)
                            <option value="{{ $soilType->value }}" {{ old('soil_type') == $soilType->value ? 'selected' : '' }}>
                                {{ $soilType->label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($fertilityOptions as $fertility)
                            <option value="{{ $fertility->value }}" {{ old('fertility') == $fertility->value ? 'selected' : '' }}>
                                {{ $fertility->label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($irrigationOptions as $irrigation)
                            <option value="{{ $irrigation->value }}" {{ old('irrigation') == $irrigation->value ? 'selected' : '' }}>
                                {{ $irrigation->label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input
                        ...
                        value="{{ old('location') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Ex: Zone Nord, Près de la route..."
                    >
                    @error('location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea
                        ...
                        rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Notes supplémentaires sur le champ..."
                    >{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
