...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Nouvelle récolte</h1>
        <p class="text-gray-600 mt-1">Enregistrez une nouvelle récolte</p>
    </div>

    <form method="POST" action="{{ route('harvests.store') }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
        ...
        <div class="p-6">
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    placeholder="Notes supplémentaires sur la récolte..."
                >{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end space-x-4">
            ...
            >
                <i class="fas fa-save mr-2"></i> Créer la récolte
            </button>
        </div>
    </form>
</div>
