...
@section('title', 'Nouvelle culture')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Nouvelle culture</h1>
        <p class="text-gray-600 mt-1">Ajoutez une nouvelle culture à votre ferme</p>
    </div>

    <form method="POST" action="{{ route('crops.store') }}" class="bg-white rounded-lg shadow-md overflow-hidden">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('variety') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Ex: Variété locale, Hybrid..."
                    >
                    @error('variety')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('planting_season') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Ex: Mars-Avril"
                    >
                    @error('planting_season')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="water_needs"
                        name="water_needs"
                        value="{{ old('water_needs') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Ex: Élevé, Modéré, Faible"
                    >
                    @error('water_needs')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('soil_requirements') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        placeholder="Ex: Sol bien drainé, pH 6-7"
                    >
                    @error('soil_requirements')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end space-x-4">
            ...
            >
                <i class="fas fa-save mr-2"></i> Créer la culture
            </button>
        </div>
    </form>
</div>
